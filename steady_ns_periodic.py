...
# Problem
problem = d3.NLBVP([u, p, tau_p], namespace=locals())
problem.add_equation("div(u) + tau_p = 0")
# problem.add_equation("grad(p) - nu*div(grad_u) + lift(tau_u2,-1) = - u @ grad(u)")
# problem.add_equation("grad(p) - nu*lap(u) = 0")
problem.add_equation("grad(p) - nu*div(grad_u) = phi*(u - U)/tau - u @ grad(u)")

problem.add_equation("integ(p) = 0") # Pressure gauge
# problem.add_equation("integ(grad(p) @ ex) = -1") # Pressure gauge
# problem.add_equation("integ(u @ ey) = 0") # Pressure gauge
